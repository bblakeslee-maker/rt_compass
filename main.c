...
int main(void){
	...
	float roll_rad, pitch_rad;					// Roll and pitch in radians
	float roll_deg, pitch_deg;					// Roll and pitch in degrees
	float mg_z2;												// Intermediary pitch variable
	...
	for(i = 0; i < 10; i++){//while(1){
		...
		if(status & ZYXDA){
			...
			pitch_rad = atan2(-mg_x, mg_z2);							// Compute pitch
